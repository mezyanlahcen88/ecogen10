...
$(document).ready(function () {
  ...
  function loadTableFromLocalStorage() {
    ...
    var inputDateId = "#status_date"; // Initialiser le datepicker sur l'élément spécifique

    flatpickr(inputDateId, {
      enableTime: true,
      dateFormat: "Y-m-d H:i",
      defaultDate: data.object.status_date // Utiliser la date à partir de 'data'
  ...
  function pushProdToListe(prod) {
    ...
    if (existingProduct) {
      ...
      console.log("pushProdToListe prix:" + existingProduct.pivot.price);
    } else {
      // If it doesn't exist, add it to the list
      listeProd.push(prod);
    }

    console.log("pushProdToListe prod prix:" + prod.pivot.price);
    localStorage.setItem('product_devisEdit', JSON.stringify(data));
  }
  ...
  function tableProducts() {
    var data = JSON.parse(localStorage.getItem('product_devisEdit')) || [];
    var listeProd = data.devisProducts;
    console.log('listeprod:', listeProd); // tableBody.empty();
  ...
  $('.getProduct').on('click', function () {
    var product_id = $('select[name="product_id"]').val(); //  console.log(product_id);
  ...
  //         products: JSON.parse(localStorage.getItem('product_devisEdit'))
  //     }
  //     $.ajaxSetup({
  //         headers: {
  //             'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
