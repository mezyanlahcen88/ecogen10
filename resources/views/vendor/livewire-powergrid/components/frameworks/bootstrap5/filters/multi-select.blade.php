...
<div x-cloak
    ...
    @if(filled($multiSelect))
        <div class="{{ $theme->baseClass }}" style="{{ $theme->baseStyle }}">
            <select data-none-selected-text="{{ trans('translation.livewire-powergrid::datatable.multi_select.select') }}"
                    multiple
                    wire:model="filters.multi_select.{{ $multiSelect['dataField'] }}.values"
                    x-ref="select_picker_{{ $multiSelect['dataField'] }}"
                    data-live-search="true">
                <option value="">{{ trans('translation.livewire-powergrid::datatable.multi_select.all') }}</option>
